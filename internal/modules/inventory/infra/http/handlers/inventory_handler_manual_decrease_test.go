...
func TestInventoryHandler_ManualDecrease(t *testing.T) {
	tests := []struct {
		name                string
		productID           string
		requestBody         string
	...
	}{
		{
			...
			expectedBodyContain: `"available_quantity":95`,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/products/"+tt.productID+"/inventory/manual-decrease", bytes.NewBufferString(tt.requestBody))
			req.SetPathValue("product_id", tt.productID)
			w := httptest.NewRecorder()

			handler.ManualDecrease(w, req)

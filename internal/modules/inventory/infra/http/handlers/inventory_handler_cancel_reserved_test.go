...
func TestInventoryHandler_CancelReserved(t *testing.T) {
	tests := []struct {
		name                string
		productID           string
		requestBody         string
		...
		{
			name:        "success - reserved stock cancelled",
			...
			expectedBodyContain: `"available_quantity":110`,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/products/"+tt.productID+"/inventory/cancel-reserved", bytes.NewBufferString(tt.requestBody))
			req.SetPathValue("product_id", tt.productID)
			w := httptest.NewRecorder()

			handler.CancelReserved(w, req)

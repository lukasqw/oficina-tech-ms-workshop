...
func TestInventoryHandler_Increase(t *testing.T) {
	tests := []struct {
		name                string
		productID           string
		requestBody         string
	...
	}{
		{
			...
			expectedBodyContain: `"available_quantity":150`,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/products/"+tt.productID+"/inventory/increase", bytes.NewBufferString(tt.requestBody))
			req.SetPathValue("product_id", tt.productID)
			w := httptest.NewRecorder()

			handler.Increase(w, req)

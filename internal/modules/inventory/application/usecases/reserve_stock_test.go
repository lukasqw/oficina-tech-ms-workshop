...
func TestReserveStockUseCase_Execute(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "sucesso ao reservar estoque com disponível suficiente",
			input: ReserveStockInput{
		...
		{
			name: "sucesso ao reservar estoque com disponível insuficiente (cria pendência)",
			input: ReserveStockInput{
				ProductID: validProductID,
				Quantity:  15,
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			useCase := NewReserveStockUseCase(tt.mockRepo)

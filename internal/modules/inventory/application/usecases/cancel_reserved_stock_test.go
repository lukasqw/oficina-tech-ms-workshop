...
func TestCancelReservedStockUseCase_Execute(t *testing.T) {
	validProductID := uuid.New().String()
	now := time.Now()

	tests := []struct {
		name          string
		input         CancelReservedStockInput
		mockRepo      *mockInventoryRepository
		expectedError error
	}{
		{
			name: "sucesso ao cancelar reserva com pendências",
			input: CancelReservedStockInput{
		...
		{
			name: "sucesso ao cancelar reserva sem pendências",
			input: CancelReservedStockInput{
				ProductID: validProductID,
				Quantity:  5,
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			useCase := NewCancelReservedStockUseCase(tt.mockRepo)
